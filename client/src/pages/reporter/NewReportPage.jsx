...
import { saveDraft, loadDraft, clearDraft } from '../../utils/offlineDraft';
...
export default function NewReportPage() {
  ...
  useEffect(() => {
    ...
    if (draft) {
      setHotel(draft.hotel || user.hotel || '');
      setRoom(draft.room || '');
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (room || description) {
        saveDraft({ hotel, room, description, urgency });
      }
    }, 500);
    return () => clearTimeout(timer);
  }, [hotel, room, description, urgency]);
